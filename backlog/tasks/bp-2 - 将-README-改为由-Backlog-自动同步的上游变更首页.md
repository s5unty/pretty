...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 README 标明派生项目身份、上游仓库及基线版本/提交，保留上游使用说明入口。
- [x] #2 变更概览仅用无序列表展示已完成任务，每项只有任务编号（有效链接）、任务名和任务类型；类型缺失时使用未分类。
- [x] #3 不展示进行中/计划/其它未完成任务及其统计，不再生成已完成的变更、正在进行、准备做的变更独立章节；任务详情保留在任务页。
- [x] #4 Backlog 成功提交后按约定机制自动生成 README，触发时机、一次性配置、本地/远端边界有说明。
- [x] #5 生成确定性、幂等，不产生提交循环，不覆盖 Backlog 或无关用户修改，失败可诊断。
- [x] #6 测试覆盖已完成列表、链接、类型降级、未完成任务排除、幂等及实际自动触发，检查与类型检查通过。
- [x] #7 个人工作流脚本、配置、钩子、依赖与专用检查集中在 backlog/workflow，说明合并现有 AGENTS.md。
- [x] #8 产品 CI、package.json、bun.lock、tsconfig.json 无工作流改动。
<!-- AC:END -->
...
本轮精简完成：变更概览仅生成已完成任务的无序列表，每项为编号链接、任务名、类型（缺失显示未分类）；已删除三处分区、未完成统计和任务详情卡片，并移除不再使用的正文/验收解析及进行中/计划状态配置。README 简介与 AGENTS 展示规则已同步。15 项工作流检查全部通过，覆盖精确列表格式、未完成排除、缺失类型、completed 链接、实际 post-commit 和修改保护；类型检查、生成一致性及 diff 检查通过。产品 CI/package/bun.lock/tsconfig 确认仍无差异。
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
README 保留上游基线，仅在变更概览以无序列表展示已完成任务的编号链接、任务名和类型；不再展示进行中/计划任务或三处分区。生成器、配置和 AGENTS 规则同步精简。个人工作流仍集中于 backlog/workflow，post-commit 只生成本地 README；产品 CI、依赖和构建不变。15 项工作流检查及类型检查通过。
